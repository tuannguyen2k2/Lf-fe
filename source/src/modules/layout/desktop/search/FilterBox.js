...
const FilterBox = ({ options, queryName, queryKey }) => {
    const [ opened, { toggle } ] = useDisclosure(true);
    const { params, setQueryParams } = useQueryParams();
    const currentParams = qs.parse(location.search);
    const handleRadioChange = (value) => {
        setQueryParams({ ...currentParams, [queryKey]: value });
    };
    ...
    return (
        <div className={styles.filter}>
            ...
            <Collapse in={opened} className={styles.lessonWrapper} mb={12}>
                <Radio.Group onChange={handleRadioChange} value={currentParams[queryKey]}>
